...
class ZbufferModelPts(nn.Module):
    ...
    def forward(self, batch):
        ...
        if self.opt.concat == True:
            #pass-the-concated-feature-with-image
            gen_fs=torch.cat((input_img,gen_fs),dim=1)
            #print("here")

        gen_img = self.projector(gen_fs)
        #print(input_img.shape,fs.shape,regressed_pts.shape,gen_fs.shape,gen_img.shape)    
